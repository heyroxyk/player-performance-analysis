...
/**
 * Fetches the current season-to-date skater stats + ratings + Portal status from the live API
 * and assembles them into a plain snapshot object -- season-resolved, trimmed, and joined, but
 * NOT fingerprinted, deduped against disk, or written anywhere (see src/snapshot.js's
 * captureSnapshot for the Node adapter that does those three things on top of this). Pulling
 * this fetch/assemble logic out from under captureSnapshot is what lets the browser's live-
 * refresh feature build an equally real snapshot with no disk and no node:crypto.
 * @param {{league: number, season?: number}} params
 * @param {typeof defaultBuildDeps} deps
 * @returns {Promise<{snapshot: {league: number, season: number, capturedAt: string, players: Array<object>}, warning: string | null}>}
 */
